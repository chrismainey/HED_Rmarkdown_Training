...
for(trust_name in trusts){
  ...
  rmarkdown::render(input = "Ex10C_RMarkdown_Report.Rmd"
...
Now make a _single_ RMarkdown document which can create two reports for each trust; one the same as the reports we just made but with the title "Length of Stay Report: <trust name>". The other report should have the title "Extended Length of Stay Report: <trust name>" which includes an extra code chunk and a plot of age vs number of deaths for that trust, grouped into the age categories 0-16, 17-64, 65+

Build the extended and non-extended report for Trust2 and Trust5 in the code chunk below.

```{r build_extended_reports, eval = FALSE}
...
# for loop to write the reports with the render function
for(trust_name in c("Trust2", "Trust5")){
  for(extended in c(0,1)){
  # render can take a lot of parameters. For this exercise we have used:
  #     input is the .Rmd file to build a report from. Use your own file or the one in Exercises/Solutions/
  #     output_file is the name of the file which will be created. Make sure it is different for each loop, or it will overwrite the existing file each time
  #     params is a LIST of named variables to pass to the report. The solution report has the variable Trust = "Trust1"
      
  rmarkdown::render(input = "Ex11B_report.Rmd"
                    , output_file = file.path("report_batch", paste("Ex11_report_"
                                                                    , trust_name
                                                                    , ifelse(extended == 0, "", "_extended")
                                                                    , sep = "")
                                              )
                    , params = list(Trust = trust_name, extended = extended)
                    )
  }
}
