...
# Exercise 2: 
### Build an `Rmarkdown` using `LOS_model`
#### Part (a)
...
+ Create a new Markdown file, or use an existing one created from earlier
+ Load the NHSRdatasets package and the `LOS_models` dataset
+ Print the table using Kable format.  
...
+ The alternative to the complicated PDF generation is the `pagedown`.

+ It doens't require the Latex translation, and paginated HTML

+ `pagedown` has templates for page html, thesis, posters, CV, business cards, journals etc.
...
| :------------: | :------:| :-------:|
| Use an existing R Markdown template I have created and tackle the areas that are commented out (Ex4_CSS_Activity_1.rmd) in the 'Exercises' folder | Produce an R Markdown with HTML and CSS from scratch that utilise these skills | Adapt an existing R Markdown file that you have used in today's sessions
...
+ `thesisdown` - Extenstion for building academic theses in Rmarkdown
+ `blogdown` - BUild websites using Rmarkdown, and the static site generator 'Hugo'
+ `pkgdown` - Turn your documentaion and help files for and R package into a website

+ `xaringan` - Rmarkdown presentations using HTML slides.  Presentation
...
# CM's website build in blogdown
